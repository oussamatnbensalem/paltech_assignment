  ...
  custom_interfaces__msg__Waypoint__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    custom_interfaces__msg__Waypoint * data =
      (custom_interfaces__msg__Waypoint *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!custom_interfaces__msg__Waypoint__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          custom_interfaces__msg__Waypoint__fini(&data[i]);
        }
        free(data);
        return false;
      }
